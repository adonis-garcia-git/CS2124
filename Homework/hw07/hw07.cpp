...
public:
	PersonWithStrengthToFight(const string& name, double strength) : Noble(name, strength) {}
...
// Constructor: initializes a Protector with a name and strength
Protector::Protector(const string& aName, double aStrength) : name(aName), strength(aStrength) {
	hiringNoble = nullptr;
	is_hired = false;
	is_dead = false;
}
...
// Marks the Protector as hired
void Protector::gotHired(Lord* aLord) { 											// confirm if const is needed
	is_hired = true;
	hiringNoble = aLord;
}
...
// Noble constructor
Noble::Noble(const string& aName, double strength) : name(aName), strength(strength) {
	is_dead = false;
}
...
void PersonWithStrengthToFight::getsHit (double ratio) {
	updateStrength(-1 * getStrength() * (1 - ratio));
}
