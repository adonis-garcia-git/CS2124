// rec13_start.cpp

#include <iostream>
#include <fstream>
#include <vector>      // task 1
#include <list>        // task 2
#include <algorithm>   // task 3
#include <string>      // task 21
#include <set>         // task 22
#include <map>         // task 23
using namespace std;

// Task 9

// void print_list_manual(const list<int>& aList){
//     for (list<int>::iterator iter = aList.begin(); iter != aList.end(); ++ ++iter) {
//         cout << *iter << " ";
//     }
//     cout << endl;
// }

void print_list_manual(const list<int>& aList){
    for (list<int>::const_iterator iter = aList.begin(); iter != aList.end(); ++iter) {
        cout << *iter << " ";
    }
    cout << endl;
}

// Task 10

void print_list_ranged(const list<int>& aList){
    for (int val : aList) {
        cout << val << " ";
    }
    cout << endl;
}

// Task 11

void print_list_auto(const list<int>& aList){
    for (auto iter = aList.begin(); iter != aList.end(); ++ ++iter) {
        cout << *iter << " ";
    }
    cout << endl;
}

int main() {
    // 1. Create a vector with some values and display using ranged for
    cout << "Task 1:\n";

    vector<int> values{1,2,7,6,3,4,8,5};
    for (int val : values){cout << val << " ";}

    cout << "\n=======\n";

    // 2. Initalize a list as a copy of values from the vector
    cout << "Task 2:\n";

    list<int> numbers(values.begin(), values.end());
    for (int val : numbers){cout << val << " ";}

    cout << "\n=======\n";

    // 3. Sort the original vector.  Display both the vector and the list
    cout << "Task 3:\n";

    sort(values.begin(), values.end());
    for (int val : values){cout << val << " ";}
    cout << endl;
    for (int val : numbers){cout << val << " ";}

    cout << "\n=======\n";

    // 4. print every other element of the vector.
    cout << "Task 4:\n";

    for (size_t i = 0; i < values.size(); ++ ++i){
        cout << values[i] << " ";
    }

    cout << "\n=======\n";

    // 5. Attempt to print every other element of the list using the
    //    same technique.
    cout << "Task 5:\n";

    // for (size_t i = 0; i < numbers.size(); ++ ++i){
    //     cout << values[i] << " ";
    // }

    cout << "\n=======\n";

    //
    // Iterators
    //

    // 6. Repeat task 4 using iterators.  Do not use auto;
    cout << "Task 6:\n";

    for (vector<int>::iterator iter = values.begin(); iter != values.end(); iter += 2) {
        cout << *iter << " ";
    }

    cout << "\n=======\n";

    // 7. Repeat the previous task using the list.  Again, do not use auto.
    //    Note that you cannot use the same simple mechanism to bump
    //    the iterator as in task 6.
    cout << "Task 7:\n";

    for (list<int>::iterator iter = numbers.begin(); iter != numbers.end(); ++ ++iter) {
        cout << *iter << " ";
    }

    cout << "\n=======\n";

    // 8. Sorting a list
    cout << "Task 8:\n";

    numbers.sort();
    for (int val : numbers){cout << val << " ";}

    cout << "\n=======\n";

    // 9. Calling the function to print the list
    cout << "Task 9:\n";

    print_list_manual(numbers);

    cout << "=======\n";

    // 10. Calling the function that prints the list, using ranged-for
    cout << "Task 10:\n";

    print_list_ranged(numbers);

    cout << "=======\n";

    //
    // Auto
    //

    // 11. Calling the function that, using auto, prints alterate
    // items in the list
    cout << "Task 11:\n";

    print_list_auto(numbers);

    cout << "=======\n";

    
    // 12.  Write a function find that takes a list and value to search for.
    //      What should we return if not found
    cout << "Task 12:\n";

    

    cout << "=======\n";

    // 13.  Write a function find that takes a list and value to search for.
    //      What should we return if not found
    cout << "Task 13:\n";

    cout << "=======\n";

    //
    // Generic Algorithms
    //

    // 14. Generic algorithms: find
    cout << "Task 14:\n";

    cout << "=======\n";

    // 15. Generic algorithms: find_if
    cout << "Task 15:\n";

    cout << "=======\n";

    // 16. Functor
    cout << "Task 16:\n";

    cout << "=======\n";
    
    // 17. Lambda
    cout << "Task 17:\n";

    cout << "=======\n";

    // 18. Generic algorithms: copy to an array
    cout << "Task 18:\n";

    cout << "=======\n";

    //
    // Templated Functions
    //

    // 19. Implement find as a function for lists
    cout << "Task 19:\n";

    cout << "=======\n";
    
    // 20. Implement find as a templated function
    cout << "Task 20:\n";

    cout << "=======\n";

    //
    // Associative collections
    //

    // 21. Using a vector of strings, print a line showing the number
    //     of distinct words and the words themselves.
    cout << "Task 21:\n";

    cout << "\n=======\n";

    // 22. Repeating previous step, but using the set
    cout << "Task 22:\n";

    cout << "=======\n";

    // 23. Word co-occurence using map
    cout << "Task 23:\n";

    cout << "=======\n";
}
