...
void splice(Node* toSplice, Node* prior){
    ...
    if (prior->next == nullptr) { // it is the last node
        prior->next = toSplice;
        return;
    }
...
const Node* isSublist(const Node* pattern, const Node* target){
    ...
    while (target){
        curr_t = target;
        curr_p = pattern;
        if (curr_t->data == curr_p->data){ // reset curr_p
            while (curr_p->next && curr_t->next) {
                if (curr_p->next->data == curr_t->next->data){
                    curr_p = curr_p->next;
                    curr_t = curr_t->next;
                    continue;
                }
            ...
            if (curr_p->next == nullptr) return target;  
        }
        target = target->next;
    }
    return nullptr;
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB){
    ...
    while (listA) {
        while (curr_b) {
            if (listA->next && curr_b){
                if (listA->next == curr_b) return curr_b;
            }
            curr_b = curr_b->next;
            }
        curr_b = listB;
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB){
    ...
    while (listA->next){
        node_set.insert(listA->next);
        listA = listA->next;
    }
    while (listB){
        if (node_set.find(listB) != node_set.end()){
            return listB;
        }
        listB = listB->next;
    }
