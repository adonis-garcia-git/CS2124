...
public:       
    ...
    void movesInWith(Person& newRoomate) {
        ...
        if (newRoomate.roomie){ // allowed to access private since its the same class type
            return;
        }
...
int main() {
    ...
    *q  = *p;      
    ...
    //  pI = &joe; // ok
    // *pI = 234; // ok
    //  pI = &sal;  //variable is const while pointer is not
    // *pI = 7623; // ok 

    // const int* pcI; // ok
    //  pcI = &joe; // ok
    // *pcI = 234; // makes joe const regardless so can't modify 
    //  pcI = &sal; // ok
    // *pcI = 7623; // can't modify a const variable

    // int* const cpI; // must be initialized
    // int* const cpI(&joe); // ok
    // int* const cpI(&sal); // sal is a const variable
    //  cpI = &joe; // can't change address of const pointer
    // *cpI = 234; // ok
    //  cpI = &sal; // can't change address of const pointer
    // *cpI = 7623; // ok

    //const int* const cpcI; // must be initialized
    //const int* const cpcI(&joe); // ok
    //const int* const cpcI(&sal); // ok
    //  cpcI = &joe; // *cpcI = 234; // can't alter anything
    //  cpcI = &sal; // can't change address
    // *cpcI = 7623; // can't modify value
    ...
    public:
        ...
        void setX( int x )  { this->x = x; } // removes ambiguity (this is &self)
    private:
        int x;
    };
    ...
    *pDyn = 17;
    ...
    delete pTest;      
    pTest = nullptr;    
    delete pTest; // safe but don't do as it was already deleted
    ...
    // delete ptrTolong;  // What happens here? // Crashes in Debug       
    Complex cmplx;      
    ...
    for (size_t ndx = 0; ndx < 3; ++ndx) {         
        delete complV[ndx]; // must do this before clearing
    }      
    // clear the vector      
    complV.clear();
